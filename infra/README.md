...
| [`openvino-vlm-kit/`](openvino-vlm-kit/README.md) | 🟢 deployed, wired to DSH | Phi-3.5-vision-instruct (INT4-ov), a **model-serving endpoint** for the separate DSH multimodality workstream — not part of the Insight Storage data flow above. Deployed on `linkhealth-openvino-vision` 2026-08-20 (resized to `n2-standard-4` for it); `dsh-vision-router`'s `httpProviders` config points at it — see [docs/deployment-gcp.md](../docs/deployment-gcp.md#vision-multimodality-dsh-vision-router--local-phi-35-vision). |
