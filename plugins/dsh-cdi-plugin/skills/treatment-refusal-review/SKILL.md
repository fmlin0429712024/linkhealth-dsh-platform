---
name: treatment-refusal-review
description: Review synthetic ICHD treatment notes for documentation of a patient refusing or discontinuing treatment early (SYN-ICHD-02, non-deterministic). Use from audit-rule-evaluation whenever a treatment_note documents a refusal/discontinuation event; judge documentation completeness against a narrative use-case description, never a rigid checklist, and never render a clinical conclusion.
...
patient treatment-refusal event gives a reviewer enough evidence to assess
it — using narrative judgment, since there is no formula for "adequately
documented."

## Use Case

During in-center hemodialysis, a patient may ask to stop or refuse to
continue a treatment before it is scheduled to end. Reasons and
appropriate responses vary by patient and context, so there is no fixed
...
1. the refusal or request to stop was recognized and recorded, including
   the stated reason if the patient gave one,
2. the care team attempted to address the patient's concerns or explained
   the risks of stopping early,
3. the physician was notified of the refusal, and
4. a follow-up or monitoring plan was documented (for example, continued
   observation and reassessment, or safe-discharge instructions if the
   patient left).
...
1. Confirm `treatment_note` documents a refusal/discontinuation event (the
   `SYN-ICHD-02` trigger in `rules/synthetic-audit-rules.md`). If not
...
intent, not a verified sandbox guarantee.
...
`allowed-tools` is architecturally implied but not confirmed with an
explicit skills-specific example. Treat this scoping as best-effort, not a
confirmed-working restriction, until tested.
