...
// apply() wiring contract, and the query_vision_events tool registration.
// query-events.test.mjs covers that tool's pure logic in depth (URL
// building, summary math, HTTP-mocked fetch) — this file only checks that
// apply() wires it up correctly against the tools/systemPrompt services.
...
test('apply() registers the prompt section and the query_vision_events tool; dispose tears both down', () => {
  ...
  assert.equal(registered.toolDescriptors.length, 1)
...
test('the registered tool execute() surfaces a fetch failure as a tool-error result, not a throw', async () => {
  const { ctx, registered } = makeCtx()
  apply(ctx, { baseUrl: 'http://backend.test:8080' })
  const tool = registered.toolDescriptors[0]
...
test('tool result renders a FAILED line when execute() returns an error', () => {
  const { ctx, registered } = makeCtx()
  apply(ctx, { baseUrl: 'http://backend.test:8080' })
  const tool = registered.toolDescriptors[0]
